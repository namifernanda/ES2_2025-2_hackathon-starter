...
Thank you!\n`,
      ...
      await nodemailerConfig.sendMail({
        ...
        errorType: "errors",
        errorMsg:
          "We encountered an issue sending instructions. Please try again later.",
        req,
      });
...
exports.postUpdateProfile = async (req, res, next) => {
  ...
  try {
    ...
    if (err.code === 11000) {
      console.log(
        "Duplicate email address when trying to update the profile email.",
      );
    } else {
  ...
  try {
    ...
    req.logout((err) => {
      ...
      req.session.destroy((err) => {
        if (err) {
          console.log(
            "Error: Failed to destroy the session during account deletion.",
            err,
          );
        }
        req.user = null;
...
exports.getReset = async (req, res, next) => {
  try {
    ...
    if (
      !user ||
      !user.verifyTokenAndIp(user.passwordResetToken, req.ip, "passwordReset")
    ) {
...
exports.getVerifyEmailToken = async (req, res, next) => {
  ...
  const validationErrors = [];
  if (
    validator.escape(req.params.token) &&
...
exports.postReset = async (req, res, next) => {
  ...
  if (
    validator.escape(req.body.password) !== validator.escape(req.body.confirm)
  ) {
    validationErrors.push({ msg: "Passwords do not match" });
  }
  if (!validator.isHexadecimal(req.params.token)) {
