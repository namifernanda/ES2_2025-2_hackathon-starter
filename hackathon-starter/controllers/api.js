...
exports.getFoursquare = async (req, res, next) => {
  try {
    ...
    const [trendingVenuesRes, venueDetailRes, venuePhotosRes] =
      await Promise.all([
        fetch(
          "https://api.foursquare.com/v3/places/search?ll=47.609657,-122.342148&limit=10",
          {
            headers,
          },
        ).then((res) => res.json()),
        ...
        fetch(
          "https://api.foursquare.com/v3/places/427ea800f964a520b1211fe3/photos",
          {
            headers,
          },
        ).then((res) => res.json()),
...
exports.getGithub = async (req, res, next) => {
  ...
  if (!req.user) {
    ...
    !req.user.tokens ||
    !req.user.tokens.find((token) => token.kind === "github")
  ) {
  ...
  if (githubToken) {
    github = new Octokit({
      auth: req.user.tokens.find((token) => token.kind === "github")
        .accessToken,
    });
    try {
      ...
      ({ data: userEvents } =
        await github.activity.listEventsForAuthenticatedUser({
  ...
  try {
    ...
    const { data: repoStargazers } =
      await github.activity.listStargazersForRepo({
        owner: "sahat",
        repo: "hackathon-starter",
        per_page: limit,
      });
...
exports.getNewYorkTimes = async (req, res, next) => {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      throw new Error(
        `New York Times API - ${response.status} ${response.statusText}`,
      );
    }
    if (!contentType.includes("application/json")) {
      ...
      throw new Error(
        "NYT API did not return JSON. Check your API key and endpoint.",
      );
    }
    const data = await response.json();
...
exports.getTwilio = (req, res) => {
  ...
  res.render("api/twilio", {
    ...
    isSandbox,
    sandboxInfoUrl:
      "https://www.twilio.com/docs/iam/test-credentials#test-sms-numbers", // Twilio sandbox info link
  });
};
...
exports.postTwilio = async (req, res) => {
  ...
  try {
    ...
    const errorMessages = {
      ...
      21606:
        'The "From" phone number is not owned by your account or is not SMS-capable.',
...
exports.getTwitch = async (req, res, next) => {
  ...
  const getUser = async (userID) => {
    ...
    if (!response.ok) {
      throw new Error(
        `There was an error while getting user data: ${response.status}`,
      );
    }
    const data = await response.json();
    return data;
  ...
  const getFollowers = async (userID) => {
    ...
    if (!response.ok) {
      throw new Error(
        `There was an error while getting followers: ${response.status}`,
      );
    }
    const data = await response.json();
    return data;
  ...
  const getStreams = async (gameID) => {
    ...
    if (!response.ok) {
      throw new Error(
        `There was an error while getting streams: ${response.status}`,
      );
    }
    const data = await response.json();
    return data;
  ...
  const getUserByLogin = async (loginID) => {
    const response = await fetch(
      `https://api.twitch.tv/helix/users?login=${loginID}`,
      {
        headers: {
          Authorization: `Bearer ${token.accessToken}`,
          "Client-ID": twitchClientID,
    ...
    if (!response.ok) {
      throw new Error(
        `There was an error while getting user info by login: ${response.status}`,
      );
    }
    const data = await response.json();
    return data;
...
exports.getChart = async (req, res, next) => {
  ...
  try {
    ...
    if (stockdata === undefined) {
      dataType =
        "Unable to get live data from Alpha Vantage. Using previously downloaded data:";
...
exports.getGoogleDrive = (req, res) => {
  ...
  const errorMsgGeneric =
    "There was an error while fetching Google Drive data.";
  drive.files.list(
    ...
    (err, response) => {
      if (err) {
        ...
        const msg =
          err.message === "Insufficient Permission"
            ? errorMsgPermission
            : errorMsgGeneric;
...
exports.getGoogleSheets = (req, res) => {
  ...
  const errorMsgGeneric =
    "There was an error while fetching Google Sheets data.";
...
async function fetchTraktUserHistory(traktToken, limit) {
  const res = await fetch(
    `https://api.trakt.tv/users/me/history?limit=${limit}`,
    {
      headers: {
        Authorization: `Bearer ${traktToken}`,
        "trakt-api-version": 2,
...
async function fetchTraktTrendingMovies(limit) {
  const res = await fetch(
    `https://api.trakt.tv/movies/trending?limit=${limit}&extended=images`,
    {
      headers: {
        "trakt-api-version": 2,
        "trakt-api-key": process.env.TRAKT_ID,
...
async function fetchMovieDetails(slug, watchers) {
  const res = await fetch(
    `https://api.trakt.tv/movies/${slug}?extended=full,images`,
    {
      headers: {
        "trakt-api-version": 2,
        "trakt-api-key": process.env.TRAKT_ID,
